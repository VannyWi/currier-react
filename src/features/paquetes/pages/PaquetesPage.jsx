...
function PaquetesPage() {
  ...
  return (
    <>
      <PageHeader
        title="Envios"
        action={canWrite(session.role) ? <button type="button" onClick={() => setCreateModalOpen(true)} className="w-full rounded-lg bg-slate-900 px-4 py-2 font-bold text-white hover:bg-slate-800 md:w-auto">Nuevo envio</button> : null}
      />
      <section className="mb-4 grid gap-3 rounded-xl border border-slate-200 bg-white p-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      {loading ? <LoadingState text="Cargando paquetes..." /> : paquetes.length ? (
        ...
        <div className="grid gap-3 md:hidden">
          {paquetes.map((paquete) => (
            <article key={paquete.id || paquete.codigoRastreo} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  <p className="text-sm font-bold text-slate-500">Codigo</p>
                  <Link to={`/paquetes/${paquete.id}`} className="break-all text-lg font-bold text-slate-950 underline-offset-4 hover:underline">{paquete.codigoRastreo || paquete.id}</Link>
                </div>
                <StatusBadge status={paquete.estado} />
              </div>
              <div className="mt-3 grid gap-2 rounded-lg bg-slate-50 p-3 text-sm">
                <p><span className="font-bold text-slate-500">Descripcion: </span><span className="font-semibold text-slate-900">{paquete.descripcion || '-'}</span></p>
                <p><span className="font-bold text-slate-500">Ruta: </span><span className="font-semibold text-slate-900">{paquete.sucursalOrigen} - {paquete.sucursalDestino}</span></p>
                <p><span className="font-bold text-slate-500">Tarifa: </span><span className="font-semibold text-slate-900">S/ {paquete.tarifa ?? '-'}</span></p>
              </div>
              <div className="mt-4 grid gap-2 sm:grid-cols-3">
                <Link to={`/paquetes/${paquete.id}`} className="rounded-md border border-slate-300 px-3 py-2 text-center font-bold text-slate-700">Ver</Link>
                {canWrite(session.role) ? <button type="button" onClick={() => openEditModal(paquete)} className="rounded-md border border-slate-300 px-3 py-2 font-bold text-slate-700">Editar</button> : null}
                {canDelete(session.role) ? <button type="button" onClick={() => setPaqueteToDelete(paquete)} className="rounded-md border border-red-200 px-3 py-2 font-bold text-red-700">Eliminar</button> : null}
              </div>
            </article>
          ))}
        </div>
